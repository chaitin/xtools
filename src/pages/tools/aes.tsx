...
const AES: React.FC = () => {
  ...
  const calc = (src: string, key: string, mode: string, iv: string) => {
    ...
    return (method === "encrypt" ? crypto.AES.encrypt : crypto.AES.decrypt)(src, key, {
    ...
    <MenuView>
      ...
      >
        ...
        <TextField
            ...
            InputProps={{
              startAdornment: <InputAdornment position="start" sx={{ width: "100px", fontFamily: "Mono" }}>{method === "encrypt" ? "加密" : "解密" }密钥</InputAdornment>,
            }}
        ...
        {mode === 'cbc' ? <TextField
          ...
          placeholder="建议为 16 位字符"
          InputProps={{
            startAdornment: <InputAdornment position="start" sx={{ width: "100px", fontFamily: "Mono" }}>IV</InputAdornment>,
          }}
          sx={{ input: { fontSize: "14px", fontFamily: "Mono" } }}
        /> : ''}
        <Stack direction="row">
          <FormLabel sx={{ lineHeight: "42px", width: "100px", color: "rgba(0, 0, 0, 0.87)"}}>模式</FormLabel>
        ...
        <Typography sx={{ marginTop: "10px" }}>{method === "encrypt" ? "明文" : "密文" }</Typography>
        <TextField
              value={src}
              variant="outlined"
              multiline
            ...
            />        
        <Typography sx={{ marginTop: "10px" }}>{method === "encrypt" ? "密文" : "明文" }</Typography>
        <TextField
          value={dst}
          variant="outlined"
          multiline
