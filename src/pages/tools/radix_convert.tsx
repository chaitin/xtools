...
const RadixConvert: React.FC = () => {
  ...
  let [valueArray, setValueArray] = useState(Array<string>(radixList.length).fill("0"));

  const onValueChange =
    (radixList: number[], value: bigInt.BigInteger) => {
  ...
  let activeElements = radixList.slice(0, splitAt).map((radix, index) => {
    return <TextField
      ...
      variant="outlined"
      InputProps={{
        startAdornment: <InputAdornment position="start" sx={{ width: "70px", fontFamily: "Mono" }}>{radix + " 进制"}</InputAdornment>,
      }}
