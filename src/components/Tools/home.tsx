  ...
  return (
    <Box sx={{ width: '100%', height: '100%', pt: 4 }}>
      <Stack direction='row' justifyContent='center' alignItems='center'>
        <Typography variant='h5' fontFamily='Mono'>
          <Typography
            fontFamily='Mono'
            variant='h5'
            component='span'
        ...
        <Paper
          sx={{
            ...
            borderRadius: '4px',
            ml: 1,
            '& .MuiInput-underline': { height: '44px', borderBottom: 'none' },
          }}
        >
          <Autocomplete
            disablePortal
      ...
      <Stack
        sx={{ width: '100%', maxWidth: '760px', mx: 'auto', mt: '50px' }}
        spacing={3}
      >
        {tags.map((section) => [
              ...
              .map((item) => (
                <Link className='custom-link' key={item.label} href={item.path}>
                  <Button
                    variant='contained'
                    size='small'
                    sx={{ borderRadius: '4px', background: primary }}
                  >
                    {item.label}
                  </Button>
                </Link>
