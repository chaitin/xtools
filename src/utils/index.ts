...
export function setCellWidthInExcel(data: any[]) {
  const wchs: number[] = Object.keys(data[0] || {}).map(it => getExcelCellWidth(it))
  data.map((it) => {
    const values: string[] = Object.values(it || {}).map(it => JSON.stringify(it))
    for (let i = 0; i < values.length; i++) {
      wchs[i] = Math.max(getExcelCellWidth(values[i]), wchs[i])
    }
  })
  const tcwchs: number[] = Object.keys(data[0] || {}).map(it => getRCTableCellWidth(it))
  data.map((it) => {
    const values: string[] = Object.values(it || {}).map(it => JSON.stringify(it))
    for (let i = 0; i < values.length; i++) {
